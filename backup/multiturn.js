...
var step = 0;
var foundersSteps = new builder.StepResolver(function (session, args) {
    ...
    if (step === 0) {
        step = 1;
        askCompany(session, args, next);
    } else {
        answerQuestion('founders', prompts.answerFounders)(session, args);
    }
});

/** Answer website related questions like "how can I contact microsoft?" */
dialog.matches('Founders', foundersSteps);
